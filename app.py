...
bot = telegram.Bot(token='193304113:AAHKVL_0Z7dLQ4vDj5PleqDZ5Z7KKjP1uJA')

@app.route('/')
def index():
    return r'{"xiaoyue_bot":"hehe"}'
...
def help(message):
    text = ('/echo - Repeat the same message back\n'
            '/milestone - Get xiaoyue_bot\'s milestone\n'
...
def milestone(message):
    ...
    now = datetime.now()
    text = 'xiaoyue_bot 和他家老婆大人已经认识并相爱 %d 天啦（此处应该有恭喜' % (now - from_day).days
    chat_id = message.chat.id
    bot.sendMessage(chat_id=chat_id, text=text)
